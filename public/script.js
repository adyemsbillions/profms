document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelectorAll(
      ".area-card, .publication-card, .metric-card, .feature-highlight"
    )
    .forEach((el) => {
      observer.observe(el);
    });
  ...
  if (window.location.pathname.includes("article-details.html")) {
    ...
    shareButtons.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        showNotification(
          `Share to ${platform} functionality would be implemented here.`,
          "info"
        );
      });
    });
    ...
    downloadButtons.forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        showNotification(
          `${action} functionality would be implemented here.`,
          "info"
        );
      });
    });
  ...
  paginationBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (!btn.disabled) {
        const direction = btn.textContent.includes("Next")
          ? "next"
          : "previous";
